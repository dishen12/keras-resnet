...
def _residual_block(block_function, filters, repetitions,is_first_layer=False,init_strides=(1,1)):
...
def basic_block_old(filters, init_strides=(1, 1), is_first_block_of_first_layer=False):
    """Basic 3 X 3 convolution blocks for use on resnets with layers <= 34.
    Follows improved proposed scheme in http://arxiv.org/pdf/1603.05027v2.pdf
    """
    def f(input):
...
def bottleneck_old(filters, init_strides=(1, 1), is_first_block_of_first_layer=False):
    """Bottleneck architecture for > 34 layer resnet.
    Follows improved proposed scheme in http://arxiv.org/pdf/1603.05027v2.pdf
...
class ResnetBuilder(object):
    ...
    @staticmethod
    def build_three_branch(input_shape, num_outputs):
        return ResnetBuilder.build_cross(input_shape, num_outputs, bottleneck, [3, 4, 6, 3])
    @staticmethod
    def build_three_branch_no_short_cut(input_shape, num_outputs):
        return ResnetBuilder.build_cross(input_shape, num_outputs, bottleneck_no_short_cut, [3, 4, 6, 3])
    @staticmethod
    def build_resnet_18(input_shape, num_outputs):
        return ResnetBuilder.build_cross(input_shape, num_outputs, basic_block_old, [2, 2, 2, 2])
    """
    @staticmethod
